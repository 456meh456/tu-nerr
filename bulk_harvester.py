...
BATCH_SIZE = 10   
SLEEP_TIME = 0.2  
SEARCH_LIMIT = 50 # Neighbors to scan per band
MAX_PAGES = 3     # Depth of search

# --- 0. SILENCE SSL WARNINGS ---
# Since we are bypassing SSL verify, this stops the console spam
...
def get_neighbors(artist, limit=50, page=1):
    """Get similar artists with pagination support"""
    ...
    try:
        # FIX: Added verify=False to bypass firewall issues
...
def get_details(artist):
    # 1. Deezer (Image/Fans) - The likely failure point
    d_data = None
    try:
        d_url = f"https://api.deezer.com/search/artist?q={artist}"
        # FIX: Added verify=False
        d_resp = requests.get(d_url, verify=False, timeout=5).json()
        if d_resp.get('data'): 
            d_data = d_resp['data'][0]
    except:
        pass # Deezer failed, we will catch it below

    if not d_data:
        # If Deezer fails, we skip because we need the Image/Fans for the UI
        # print(f"      [Skip] {artist} (Not found on Deezer)")
        return None
    ...
    for seed_artist in source_artists:
        ...
        while added_for_this_seed < 2 and page <= MAX_PAGES:
            ...
            for candidate in candidates:
                if added_for_this_seed >= 2:
                    break
                    
                if candidate.strip().lower() in existing_artists:
                    continue
